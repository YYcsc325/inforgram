...
class DragCanvas extends Component {
    ...
    render() {
        ...
        if (ctx) {
            ...
            // 画四个坐标线
            if (mapX[dragTarget.left]) {          // 拖拽元素的左边对齐, 画纵线, 取到对齐目标中top跟bottom的值进行比较，取最大最小
                const { min, max } = getMaxMin(dragTarget, mapX[dragTarget.left], allPosition, 'top', 'bottom');
                ctx.beginPath();
                ...
                ctx.stroke();
            } else if(mapX[dragTarget.right]){    // 拖拽元素的右边对齐
                const { min, max } = getMaxMin(dragTarget, mapX[dragTarget.right], allPosition, 'top', 'bottom');
                ctx.beginPath();
                ...
                ctx.strokeStyle = 'rgba(255,0,0,0.5)';
                ctx.stroke();
            }else if(mapY[dragTarget.top]){       // 拖拽元素的上边对齐
                ctx.beginPath();
                const { min, max } = getMaxMin(dragTarget, mapY[dragTarget.top], allPosition, 'left', 'right');
                ctx.beginPath();
                ...
                ctx.strokeStyle = 'rgba(255,0,0,0.5)';
                ctx.stroke();
            }else if(mapY[dragTarget.bottom]){    // 拖拽元素的下面对齐
                const { min, max } = getMaxMin(dragTarget, mapY[dragTarget.bottom], allPosition, 'left', 'right');
                ctx.beginPath();
